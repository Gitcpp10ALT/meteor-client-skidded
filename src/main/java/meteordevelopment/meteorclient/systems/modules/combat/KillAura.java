...
public class KillAura extends Module {
    ...
    private final SettingGroup sgStrafe = settings.createGroup("Strafe");
    ...
    private final Setting<Boolean> strafe = sgStrafe.add(new BoolSetting.Builder()
        .name("strafe")
        .description("Strafes around the target while attacking.")
        .defaultValue(false)
        .build()
    );
    ...
    private final Setting<Double> strafeSpeed = sgStrafe.add(new DoubleSetting.Builder()
        .name("strafe-speed")
        .description("The speed at which to strafe around the target.")
        .defaultValue(0.3)
        .min(0.1)
        .sliderMax(1)
        .visible(strafe::get)
    ...
    private final Setting<StrafeDirection> strafeDirection = sgStrafe.add(new EnumSetting.Builder<StrafeDirection>()
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        // Handle target strafing
        if (strafe.get() && primary != null) {
            handleStrafe(primary);
        }
    ...
    private void handleStrafe(Entity target) {
        ...
        if (distance > desiredDistance) {
            // Move towards target while strafing
            moveDirection = new Vec3d(
                Math.cos(strafeAngle) * currentSpeed + (deltaX / distance) * currentSpeed,
                0,
                Math.sin(strafeAngle) * currentSpeed + (deltaZ / distance) * currentSpeed
            );
        } else if (distance < desiredDistance - 0.5) {
            // Move away from target while strafing
            moveDirection = new Vec3d(
                Math.cos(strafeAngle) * currentSpeed - (deltaX / distance) * currentSpeed,
                0,
                Math.sin(strafeAngle) * currentSpeed - (deltaZ / distance) * currentSpeed
            ...
            // Maintain distance, only strafe
            moveDirection = new Vec3d(
                Math.cos(strafeAngle) * currentSpeed,
                0,
